...
def set_pooling(
    batch: Tensor, index: List[int], mode: str, expand: bool = False
    ...
    if expand:
        ...
        assert index == new_index, f"{index=}, {new_index=}"
    else:
        output, new_index = squeeze_from_set(x)
...
class SetPooling(nn.Module):
    def __init__(self, mode: str, expand: bool = False) -> None:
        super().__init__()
        self.mode = mode
        self.expand = expand

    def forward(self, batch: Tensor, index: List[int]) -> Tensor:
        output = set_pooling(batch, index, self.mode, self.expand)
        return output
